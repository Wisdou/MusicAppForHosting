document.addEventListener("DOMContentLoaded",  async () => {
    ...
    await loadFavorites(1, defaultPageSize);
});

async function loadFavorites(pageIndex, pageSize) {
    try {
        const response = await fetch(`${ apiBaseUrl }/tracks/favorites?pageIndex=${ pageIndex }&pageSize=${ pageSize }`, {
            method: "GET",
            credentials: "include",
        });
...
async function removeFromFavorites(trackId) {
    try {
        ...
        if (response.status === 200) {
            alert("Трек успешно убран из избранного.");
